 ...
 * 根据结果统计得到相关的代理商情况
 */
@Component("dailyCustDetailConsumeTask")
public class DailyCustDetailConsumeTask {
	private final static Logger logger = LoggerFactory.getLogger(DailyCustDetailConsumeTask.class);
	...
	public void dailyReportHandle() {
		...
		baseTaskParams.setTaskType(TaskJobConstant.TASK_TYPE_DAILYCUSTCONSUME);
		baseTaskParams.setTaskName(TaskJobConstant.TASK_NAME_DAILYCUSTCONSUME);
		...
		taskRecord.setTaskName(TaskJobConstant.TASK_NAME_DAILYCUSTCONSUME);
		taskRecord.setTaskType(TaskJobConstant.TASK_TYPE_DAILYCUSTCONSUME);
		...
		try {
			custInfoService.generateDailyCusEmptyConsume(statisticsDate);
