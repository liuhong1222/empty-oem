...
public class RedisClient {
 ...
 // 存对象
    public String setObject(String key, Object obj) throws Exception {
    	String result = "";
        try (Jedis jedis = jedisPool.getResource()) {
            ObjectOutputStream oos = null;  //对象输出流
            ByteArrayOutputStream bos = null;  //内存缓冲流
...
//            jedis.expire(key, 30 * 60 * 1000);
    ...
    public long del(String key) {
        long result = 0;
        try{
            result = jedisPool.getResource().del(key);
        } catch (Exception e) {
            log.error("jedis del 出错,key[{}],info:", key,e);
            dingDingMessage.sendMessage(String.format("警告：jedis del 出错,key[%s],e:[%s]", key,e));
            throw new RuntimeException(e);
        }
        //返还到连接池
        return result;
